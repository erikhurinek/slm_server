...
class Application:
    def __init__(self):
        ...
        logger : logging.Logger, optional
            A logger instance for logging messages. If None, a default logger
            is created.
        """
        ...
        with self.app.app_context():
            ...
            self.reset_messages()
            self.clients = {}
            self.ai_chat_generator = AiChatGenerator(
                self.slm_token_callback, self.slm_finish_callback)
            ...
            if Settings.get("context_modules"):
                ContextManager.register_context_modules(
    ...
    def reset_messages(self) -> None:
        ...
        """
        self.messages = [Message("system", Settings.get(
            "system_prompt"), 0, None, Settings.get("hide_system_prompt"))]
        self.messageId = 1
    ...
    def slm_token_callback(self, new_content: str, full_response: str) -> None:
        """
        Updates the AI message indicated by `ai_chat_target_index` with the
        ...
        if self.ai_chat_target_index is None:
            Logger.warning(
                "AI chat target index is None, cannot update message")
            return
        self.messages[self.ai_chat_target_index].content = full_response
        self.update_clients_append(
            self.messages[self.ai_chat_target_index].id, new_content)
    ...
    def slm_finish_callback(self, full_response: str) -> None:
        ...
        """
        # Update message only if an index exists. The referenced message may
        # have been deleted.
    ...
    def request_slm_background_response(self):
        """
        Request a background response for the current messages from the AI
        model.
        """
        Logger.info("Requesting AI response")
        if not self.ai_enabled:
            Logger.warning(
                "AI responses are disabled, not requesting response")
    ...
    def update_clients_append(self,
        ...
        clientIds : list of int, optional
            List of client IDs to send the messages to. If None, all clients
            are updated.
        """
        data = [self.messages[message_id].to_dict(
            Settings.get("show_user_name"))]
        data[0]["content"] = content_to_append
        data[0]["append"] = True
        clientIds = client_ids or self.clients.keys()

        if message_id >= len(self.messages) or not self.messages[message_id]:
            Logger.warning(f"Message ID {message_id} not found, not appending")
            return
        elif self.messages[message_id].hidden:
            Logger.warning(f"Message ID {message_id} is hidden, not appending")
            return

        for clientId in clientIds:
            if clientId in self.clients:
                self.socketio.emit("update_messages", {
                                   "messages": data}, to=clientId)
            else:
                Logger.warning(
    ...
    def update_clients(self, messageIds=None, clientIds=None):
        ...
        messageIds : list of int
            Message IDs to send to the clients. If None, the last message ID is
            used.
        clientIds : list of int
            List of client IDs to send the messages to. If None, all clients
            are updated.
        append : bool
            If True, indicates that the message content should be appended to
        ...
        for clientId in clientIds:
            if clientId in self.clients:
                ...
                if updated_messages:
                    self.socketio.emit("update_messages", {
                                       "messages": updated_messages}, to=clientId)
    ...
    def make_routes(self):
        ...
        def handle_connect(auth=None):
            ...
            self.socketio.emit(
                "ai_status", {"busy": self.ai_busy}, to=request.sid)
            self.socketio.emit("ai_toggle_status", {
                               "enabled": self.ai_enabled}, to=request.sid)
        ...
        def handle_provide_context():
            ...
            self.add_message("assistant", Settings.get(
                "context_pretext"), "assistant")
            self.add_message(
                "user", ContextManager.get_context(), "context", True)
            self.update_clients(
                messageIds=[self.messageId - 2, self.messageId - 1])
            self.request_slm_background_response()
