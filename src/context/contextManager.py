...
class ContextManager:
    ...
    def register_context_modules(cls, context_modules: list[dict]) -> None:
        ...
        context_modules : list[dict]
            ...
            Each module entry should contain a `name` field that matches the name of the module file, and any additional arguments.
        """

        module_folder = os.path.join(Settings.get_root(), "src", "context", "modules")

        for data in context_modules:
            if not "name" in data:
                raise KeyError("Context module data must contain a 'name' field.")

            module_name = data["name"]
            module_path = os.path.join(module_folder, f"{module_name}.py")
            if not os.path.exists(module_path):
                raise FileNotFoundError(f"Context module {module_name} not found at {module_path}")

            module = __import__(f"src.context.modules.{module_name}", fromlist=[module_name])
            ...
            if not issubclass(context_class, BaseContextModule):
                raise TypeError(f"Class {class_name} in module {module_name} must inherit from BaseContextModule")
